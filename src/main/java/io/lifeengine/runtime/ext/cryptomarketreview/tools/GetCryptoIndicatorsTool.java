...
@ConditionalOnProperty(
        ...
        havingValue = "true",
        matchIfMissing = true)
public class GetCryptoIndicatorsTool implements ToolExecutor {

    public static final String TOOL_ID = "cryptobot.get-crypto-indicators";
    private static final int DEFAULT_LIMIT = 12;

    private final CryptobotServiceClient client;
    private final ObjectMapper mapper;

    public GetCryptoIndicatorsTool(CryptobotServiceClient client, ObjectMapper mapper) {
        this.client = client;
        this.mapper = mapper;
    }
    ...
    public ToolDefinition definition() {
        return new ToolDefinition(TOOL_ID, "Recent indicator snapshots (RSI/EMA/...) for a symbol");
    }
    ...
    public Mono<ToolExecutionResult> execute(ToolExecutionRequest request, WorkflowRunContext ctx) {
        ...
        return client.getIndicators(symbol, DEFAULT_LIMIT)
                .map(arr -> {
