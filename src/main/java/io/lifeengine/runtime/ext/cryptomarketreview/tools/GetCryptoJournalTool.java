...
@ConditionalOnProperty(
        ...
        havingValue = "true",
        matchIfMissing = true)
public class GetCryptoJournalTool implements ToolExecutor {

    public static final String TOOL_ID = "cryptobot.get-crypto-journal";
    private static final int DEFAULT_LIMIT = 5;

    private final CryptobotServiceClient client;
    private final ObjectMapper mapper;

    public GetCryptoJournalTool(CryptobotServiceClient client, ObjectMapper mapper) {
        this.client = client;
        this.mapper = mapper;
    }
    ...
    public ToolDefinition definition() {
        return new ToolDefinition(TOOL_ID, "Recent operator trade-journal entries for a symbol");
    }
    ...
    public Mono<ToolExecutionResult> execute(ToolExecutionRequest request, WorkflowRunContext ctx) {
        ...
        return client.getJournal(symbol, DEFAULT_LIMIT)
                .map(arr -> {
